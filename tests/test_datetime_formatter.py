...
def test_days(date, day):
    ...
    assert "2018-01-21  " == _datetime_formatter(
        date, chars=12, delta=day * 15, left=True
    )
...
def test_day_times(date, day):
    ...
    assert "21T15:03:12 " == _datetime_formatter(
        date, chars=12, delta=day * 5, left=True
    )
...
def test_times(date, hour):
    ...
    assert "15:03:12.001234 " == _datetime_formatter(
        date, chars=16, delta=hour * 5, left=True
    )
