...
def test_ellipse_formula(n, x_amp, y_amp):
    ...
    x, y = plt_data.ellipse(
        x_center, y_center, x_amplitude=x_amp, y_amplitude=y_amp, n=n
    )
    ...
    for idx1 in range(0, len(x)):
        ...
        assert (x1 - x_center) ** 2 / x_amp**2 + (
            y1 - y_center
        ) ** 2 / y_amp**2 == pytest.approx(1, abs=0.0001)


@pytest.mark.parametrize("angle", [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75])
def test_ellipse_rotation(angle):
    x1, y1 = plt_data.ellipse(
        0, 0, angle=angle * math.pi, x_amplitude=2, y_amplitude=1, n=100
    )
    fig1 = plt.Figure()
    ...
    x2, y2 = plt_data.ellipse(
        0, 0, angle=(2 + angle) * math.pi, x_amplitude=2, y_amplitude=1, n=100
    )
    fig2 = plt.Figure()
...
def test_circle(radius):
    X, Y = plt_data.circle(0, 0, radius=radius**10, n=100)
    # complete
...
def test_ellipse_rotation_show(cleandoc):
    x1, y1 = plt_data.ellipse(
        0, 0, angle=0.25 * math.pi, x_amplitude=2, y_amplitude=1, n=100
    )
    fig = plt.Figure()
