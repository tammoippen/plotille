...
def test_names(tty):
    ...
    for i, fg in enumerate(names):
        ...
        assert "\x1b[1;{}m \x1b[0m".format(30 + i) == clr.color(
        ...
        for j, bg in enumerate(names):
            ...
            assert "\x1b[1;{}m".format(40 + j) == clr._names(
                None, "bright_" + bg + "_old"
            )

            assert "\x1b[{}m \x1b[0m".format(40 + j) == clr.color(" ", None, bg)
            assert "\x1b[{}m \x1b[0m".format(100 + j) == clr.color(
                " ", None, "bright_" + bg
            )
            assert "\x1b[1;{}m \x1b[0m".format(40 + j) == clr.color(
                " ", None, "bright_" + bg + "_old"
            )

            assert "\x1b[{};{}m".format(30 + i, 40 + j) == clr._names(fg, bg)
            assert "\x1b[{};{}m".format(90 + i, 100 + j) == clr._names(
            ...
            assert "\x1b[{};{}m \x1b[0m".format(30 + i, 40 + j) == clr.color(
                " ", fg, bg
            )
            assert "\x1b[{};{}m \x1b[0m".format(90 + i, 100 + j) == clr.color(
                " ", "bright_" + fg, "bright_" + bg
            )
...
def test_bytes(tty):
    ...
    for fg in range(0, 256):
        ...
        assert "\x1b[38;5;{}m \x1b[0m".format(fg) == clr.color(
            " ", fg, None, mode="byte"
        )

        for bg in range(0, 256):
            assert "\x1b[48;5;{}m".format(bg) == clr._byte(None, bg)
            assert "\x1b[48;5;{}m \x1b[0m".format(bg) == clr.color(
                " ", None, bg, mode="byte"
            )

            assert "\x1b[38;5;{};48;5;{}m".format(fg, bg) == clr._byte(fg, bg)
            assert "\x1b[38;5;{};48;5;{}m \x1b[0m".format(fg, bg) == clr.color(
                " ", fg, bg, mode="byte"
            )
...
def test_rgb(tty):
    ...
    for fg in range(0, 256):
        ...
        for bg in range(0, 256):
            ...
            assert "\x1b[48;2;{};{};{}m \x1b[0m".format(bg, bg, bg) == clr.color(
